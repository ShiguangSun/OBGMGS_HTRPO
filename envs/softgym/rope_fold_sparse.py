...
import time
import datetime
# from softgym.sparse_envs.rope_pick_sparse import RopePickSparseEnv
from .rope_pick_sparse import RopePickSparseEnv
import pdb

class RopeFoldSparseEnv(RopePickSparseEnv):
    ...
    def _sample_goal(self):
        ...
        # pdb.set_trace()
        # x_split = np.random.randint(int(rope_particle_num*0.3), int(rope_particle_num*0.5))
        x_split = self.np_random.randint(int(rope_particle_num*0.3), int(rope_particle_num*0.5))
        group_a_idx = particle_idx[:x_split]
        group_b_idx = np.flip(particle_idx)[rope_particle_num - 2*x_split:rope_particle_num - x_split]

        curr_pos = pyflex.get_positions().reshape((-1, 4))
        
        curr_pos[group_a_idx, :] = curr_pos[group_b_idx, :].copy()
        curr_pos[group_a_idx, 1] += 0.05
        ...
        goal = self._normalize_points(curr_pos[self.key_point_indices,:3].flatten())
